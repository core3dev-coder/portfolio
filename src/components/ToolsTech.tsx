      ...
      <div className="container px-6 space-y-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {tools.map((tool) => (
            <div
              key={tool}
              className="rounded-2xl border border-border/70 bg-card px-6 py-4 text-center text-sm font-semibold tracking-wide uppercase text-muted-foreground hover:border-primary/40 hover:text-foreground transition"
            >
              {tool}
            </div>
          ))}
